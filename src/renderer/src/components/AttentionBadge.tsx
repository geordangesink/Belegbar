 ...
 *  - minor     → yellow ring       (unimportant readings are uncertain)
 *  - warning   → yellow triangle   (potentially tax-relevant problem)
 *  - critical  → red triangle      (real concern; excluded from totals)
 *
...
/** Shape per tier: check / ring / triangle — distinguishable without color. */
function AttentionGlyph({ level, size }: { level: AttentionLevel; size: number }): ReactNode {
  ...
  // warning + critical: triangle with exclamation mark
  return (
    <svg width={size} height={size} viewBox="0 0 16 16" fill="none" aria-hidden="true">
      <path
